	...
	public function render() {
		...
		if($feature_type == 'icon') {
			...
			if($feature_image) {
				...
				if( ($title_url && $url_appear == 'icon') || ($title_url && $url_appear == 'both' ) ) $media .= '<a href="'. $title_url .'"'.($link_open_new_window ? ' rel="noopener noreferrer" target="_blank"' : '').'>';
				$media  .= '<img class="sppb-img-responsive'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $feature_image_link) . '" alt="'.strip_tags($title).'" '.($placeholder ? 'data-large="'.$feature_image_link.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy">';
				if(($title_url && $url_appear == 'icon') || ($title_url && $url_appear == 'both' )) $media .= '</a>';
				$media  .= '</span>';
			}
		}
