		...
		if(isset($this->addon->settings->sp_gallery_item) && count((array) $this->addon->settings->sp_gallery_item)){
			foreach ($this->addon->settings->sp_gallery_item as $key => $value) {
				if(isset($value->thumb) && $value->thumb) {
					...
					$output .= '<img class="sppb-img-responsive'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $value->thumb) . '" alt="' . (isset($value->title) ? $value->title : '') . '" '.($placeholder ? 'data-large="'.$value->thumb.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy">';
					$output .= ($value->full) ? '</a>' : '';
					$output .= '</li>';
				}
			}
