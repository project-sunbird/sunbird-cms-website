...
class SppagebuilderAddonImage extends SppagebuilderAddons{
	...
	public function render() {
		...
		if($image) {
			...
			$output .= '<img class="sppb-img-responsive'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $image) . '" '.($placeholder ? 'data-large="'.$image.'"' : '').' alt="'. $alt_text .'" title="'.$title.'" '.($dimension ? $dimension : '').' loading="lazy">';
