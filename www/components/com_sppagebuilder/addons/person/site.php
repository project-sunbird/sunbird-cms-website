	...
	public function render() {
		...
		if($facebook || $twitter || $youtube || $linkedin || $pinterest || $flickr || $dribbble || $behance || $instagram) {
			...
			if($image) {
					...
					$output  .= '<img class="sppb-img-responsive'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $image) . '" alt="'. $name .'" '.($placeholder ? 'data-large="'.$image.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy">';
