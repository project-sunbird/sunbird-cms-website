		...
		foreach ($settings->sp_tab_item as $key => $tab) {
            ...
            if(isset($tab->image_or_icon) && $tab->image_or_icon == 'image'){
                if($tab->image && $nav_image_postion == 'top'){
                    $image_top = '<img class="sppb-tab-image tab-image-block'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $tab->image) . '" alt="'.trim(strip_tags($title)).'" '.($placeholder ? 'data-large="'.$tab->image.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy"/>';
                } elseif ($tab->image && $nav_image_postion == 'bottom') {
                    $image_bottom = '<img class="sppb-tab-image tab-image-block'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $tab->image) . '" alt="'.trim(strip_tags($title)).'" '.($placeholder ? 'data-large="'.$tab->image.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy"/>';
                } elseif ($tab->image && $nav_image_postion == 'right') {
                    $image_right = '<img class="sppb-tab-image'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $tab->image) . '" alt="'.trim(strip_tags($title)).'" '.($placeholder ? 'data-large="'.$tab->image.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy"/>';
                } else {
                    $image_left = '<img class="sppb-tab-image'.($placeholder ? ' sppb-element-lazy' : '').'" src="' . ($placeholder ? $placeholder : $tab->image) . '" alt="'.trim(strip_tags($title)).'" '.($placeholder ? 'data-large="'.$tab->image.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy"/>';
                }
            } else {
