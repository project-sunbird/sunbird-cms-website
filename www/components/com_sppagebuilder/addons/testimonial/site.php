...
class SppagebuilderAddonTestimonial extends SppagebuilderAddons {
	...
	public function render() {
		...
		if($designation_position == 'top'){
			...
			$output .= $avatar ? '<img src="' . ($placeholder ? $placeholder : $avatar_img) . '" class="'. $avatar_shape .' sppb-addon-testimonial-avatar'.($placeholder ? ' sppb-element-lazy' : '').'" alt="'.$name.'" '.($placeholder ? 'data-large="'.$avatar_img.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy">' : '';
			$output .= '<span>';
