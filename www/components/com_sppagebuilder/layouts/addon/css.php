...
if(isset($addon->settings->global_margin)){
    if( is_object( $addon->settings->global_margin ) ) {
        $position_css .= SppagebuilderHelperSite::getPaddingMargin($addon->settings->global_margin, 'margin');
...
$inlineCSS .= "@media (min-width: 768px) and (max-width: 991px) {";
    ...
    $position_style_sm .= (isset($addon->settings->global_margin_sm) && $addon->settings->global_margin_sm) ? SppagebuilderHelperSite::getPaddingMargin($addon->settings->global_margin_sm, 'margin') : '';
...
$inlineCSS .= "@media (max-width: 767px) {";
    ...
    $position_style_xs .= (isset($addon->settings->global_margin_xs) && $addon->settings->global_margin_xs) ? SppagebuilderHelperSite::getPaddingMargin($addon->settings->global_margin_xs, 'margin') : '';
...
if( class_exists('SppbCustomCssParser') && isset($addon->settings->global_custom_css) && !empty($addon->settings->global_custom_css)){
    $inlineCSS .= SppbCustomCssParser::getCss($addon->settings->global_custom_css, $addon_id, $addon_wrapper_id);
}
