	...
	public function render() {
		...
		if($modal_selector=='image') {
			if ($selector_image) {
				...
				$output .= ' <h3 class="sppb-addon-title">' . $selector_text . '</h3> <a class="sppb-modal-selector sppb-magnific-popup" '. $attribs .' href="'. $url . '" id="'. $modal_unique_id .'-selector"><img '.($placeholder ? 'class="sppb-element-lazy"' : '').' src="' . ($placeholder ? $placeholder : $image_link) . '" alt="'. $alt_for_button .'" '.($placeholder ? 'data-large="'.$image_link.'"' : '').' '.($dimension ? $dimension : '').' loading="lazy">';
					//$output  .= ($selector_text) ? : '';
				$output  .= '</a>';
			}
